...
class AttentionControl(abc.ABC):
    ...
    def __call__(self, attn, is_cross: bool, place_in_unet: str):
        # print(f"second: {attn._version}")
        if self.cur_att_layer >= self.num_uncond_att_layers:
            h = attn.shape[0]
            attn[h // 2 :] = self.forward(attn[h // 2 :], is_cross, place_in_unet)
            # print(f"third: {attn._version}")
...
def count_training_params(model: Union[torch.nn.Module, List[torch.nn.Module]], integer: bool = False):
    if not isinstance(model, list):
        model = [model]
    params: int = 0
    for m in model:
        params = params + sum(p.numel() for p in m.parameters() if p.requires_grad)
    return f"{params / 1e6:.2f}M" if not integer else params
